...
class MainActivity : AppCompatActivity(),MyInterface.DataView,MyInterface{
    ...
    override fun requestPermission()
    {
        ...
        if ((ActivityCompat.shouldShowRequestPermissionRationale(this@MainActivity, permission[0]) &&
                    ActivityCompat.shouldShowRequestPermissionRationale(this@MainActivity, Manifest.permission.ACCESS_COARSE_LOCATION)))
        {
            ...
            alert.setMessage("The Following Permissions Needs Acceptance Please").setPositiveButton("OK")
            { dialog, which ->
        ...
        when (1.also { requestCode = it }) {
            REQUEST_CODE -> {
                if (grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    Toast.makeText(
                        this@MainActivity,
                        grantResults[0].toString() + "",
                        Toast.LENGTH_SHORT
                    ).show() //value of permission if granted
                    Toast.makeText(
                        this@MainActivity,
                    ...
                    Toast.makeText(
                        ...
                        grantResults[0].toString() + "",
                        Toast.LENGTH_SHORT
                    ).show() ///value of permission if denied
                    Toast.makeText(
                        this@MainActivity,
                        "Permission For Location Is Denied",
                        Toast.LENGTH_SHORT
                    ).show()
                }
            }
                ...
                == PackageManager.PERMISSION_GRANTED )//-3
            {
                locationManager!!.requestLocationUpdates(LocationManager.NETWORK_PROVIDER, 0, 0f,
                    object : LocationListener { override fun onLocationChanged(location: Location) {
                            Toast.makeText(this@MainActivity, "Latitude:" + location.latitude, Toast.LENGTH_SHORT).show()
                            Toast.makeText(this@MainActivity, "Longitude:" + location.longitude, Toast.LENGTH_SHORT).show()
                            Log.d("Success", "onLocationChanged: " + location.latitude)
                            Log.d("Success", "onLocationChanged: " + location.longitude)
                            giveLocationDataToPresenter(location.latitude.toString(), location.longitude.toString())
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?)
    {
        ...
        if (requestCode ==REQUEST_CHECK_SETTINGS) //if the result code related to the resultcode i'm using for GPS Enabling that i defined as
        //    public static final int REQUEST_CHECK_SETTINGS=12; //any value i want
        {
            when (resultCode) {
                RESULT_OK -> Toast.makeText(this, "GPS Is Turned On", Toast.LENGTH_SHORT).show()
                RESULT_CANCELED -> {
                }
            }
        }
